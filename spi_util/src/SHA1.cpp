...
void SHA1::Transform(uint32_t* pState, const unsigned char* pBuffer)
{
    SPI_UTIL_CLOCK_FUNCTION();
...
void SHA1::Update(const unsigned char* pbData, uint32_t uLen)
{
    SPI_UTIL_CLOCK_FUNCTION();
...
std::string SHA1::Final()
{
    ...
    for (i = 0; i < 20; ++i)
    {
        digest[i] = static_cast<unsigned char>((m_state[i >> 2] >> ((3 -
            (i & 3)) * 8)) & 0xFF);
    }

    return SHADigest2Hex(&digest[0], 20);
}
