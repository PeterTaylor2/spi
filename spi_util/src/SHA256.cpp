...
// this code was copied from libcurl sha256.c with two changes:
// 1. CURLMIN was changed to std::min
// 2. HAVE_LONGLONG has been undefined - hence __int64 is used instead
...
void SHA256::Update(const unsigned char* in, uint32_t inlen)
{
    if (SHA256_Update(&state, in, inlen) != 0)
        SPI_UTIL_THROW_RUNTIME_ERROR("Failed");
}

std::string SHA256::Final()
{
    unsigned char hash[32];
    if (SHA256_Final(&hash[0], &state) != 0)
        SPI_UTIL_THROW_RUNTIME_ERROR("Failed");

    return SHADigest2Hex(&hash[0], 32);
}
