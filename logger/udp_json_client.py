...
"""
Simple UDP JSON client that sends JSON log lines to a UDP server.
...
Behavior:
 - Reads JSON lines from a file or stdin (use --file - for stdin) or generates simple rows.
...
def parse_args():
    p = argparse.ArgumentParser(description="UDP JSON client")
    ...
    p.add_argument("--file", help="Path to JSON file to send, or '-' for stdin. If omitted use --generate.", default=None)
