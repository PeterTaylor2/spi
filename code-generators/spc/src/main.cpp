    ...
    bool verbose)
{
    ...
    CServiceConstSP service = CService::Make(serviceDoc, options);
    ...
    fns.insert(service->writePublicHeaderFile(dirname, modules));
    fns.insert(service->writePrivateHeaderFile(dirname, modules));
    if (!service->sharedService())
        fns.insert(service->writeSourceFile(dirname));

    tidyup(serviceDoc, dirname, fns);
    tidyup(serviceDoc, spi_util::path::join(dirname.c_str(), "src", NULL), fns);
...
int main(int argc, char* argv[])
{
    ...
    const char* longOptions = "noGeneratedCodeNotice license licenseFile= backup recording";
    try
    {
    ...
    try
    {
        int status = run(infilename, outfilename, dirname, options, verbose);
        return status;
    }
    catch (std::exception &e)
    {
