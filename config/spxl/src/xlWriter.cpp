    ...
    if (!m_service->sharedService)
    {
        ...
        if (!m_options.noObjectFuncs)
        {
            ostr << m_import << "\n"
                ...
                << "->ObjectToString(handle, format, options, metaData);\n"
                << "}\n"
                << "\n"
                << m_import << "\n"
                ...
                << "XLOPER* options, XLOPER* metaData)\n"
                << "{\n"
                << "    return " << xlServiceName
                << "->ObjectToFile(handle, fileName, format, options, metaData);\n"
                << "}\n"
                << "\n"
                << m_import << "\n"
...
std::string ExcelService::writeVbaFile(const std::string& dirname) const
{
    ...
    ostr << "Attribute VB_Name = \"" << m_service->name
         << "_load_addins\"\n"
         << "\n"
         << "Public Const XLL = \"xl_" << m_service->name << ".xll\"\n"
         << "Option Explicit\n"
         << "\n"
         ...
         << "'\n"
         << "    myPath = Dirname(ThisWorkbook.FullName)\n"
         << "    curPath = CurDir()\n"
         << "    ChDrive myPath\n"
         << "    ChDir myPath\n"
         << "\n"
         ...
         << "    End If\n"
         << "\n"
         << "    " << m_service->ns << "AddMenu\n"
         << "End Sub\n"
         << "\n"
         << "Public Sub Auto_Close()\n"
         << "    " << m_service->ns << "RemoveMenu\n"
