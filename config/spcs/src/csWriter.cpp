...
std::string CService::writeServiceFile(const std::string& dirname) const
{
    ...
    if (!m_service->sharedService)
    {
        ...
        ostr << "\n"
            ...
            << "    IntPtr v, int N, int[] data);\n"
            << "\n"
            << m_spiImport << "\n"
            << "private static extern int spi_Enum_Vector_set_data(\n"
            << "    IntPtr v, int N, int[] data);\n"
            << "\n"
            << m_spiImport << "\n"
            ...
            << "\n"
            << m_spiImport << "\n"
            << "private static extern int spi_Enum_Matrix_get_data(\n"
            << "    IntPtr v, int nr, int nc, int[,] data);\n"
            << "\n"
            << m_spiImport << "\n"
            << "private static extern int spi_Enum_Matrix_set_data(\n"
            << "    IntPtr v, int nr, int nc, int[,] data);\n"
            << "\n"
            << m_spiImport << "\n"
    ...
    size_t indent) const
{
    ...
    for (size_t i = 0; i < func->inputs.size(); ++i)
    {
        ...
        ostr << sep << spaces << gap;
        sep = ",\n";
        ostr << CDataType(arg->dataType, service).csType(arg->arrayDim, arg->isOptional, true) << " "
    ...
    const spdoc::Class* cls) const
{
    ...
    if (!cls->noMake)
    {
        ...
        for (size_t i = 0; i < cls->attributes.size(); ++i)
        {
            ...
            ostr << sep;
            sep = sep2;
            ostr << CDataType(attr->dataType, service).csType(attr->arrayDim, attr->isOptional, true) << " " << service->rename(attr->name, true);
        }
        ostr << ")\n"
            << "    {\n";
    ...
    if (cls->isAbstract)
    {
        ...
        {
            ostr << "\n"
                << "    public new static " << cls->name << nullable << " WrapNullable(IntPtr self)\n"
                << "    {\n"
                << "        if (self == IntPtr.Zero)\n"
                << "            return null;\n"
                << "        return Wrap(self);\n"
    ...
    else
    {
        ...
        {
            ostr << "\n"
                << "    public new static " << cls->name << "? WrapNullable(IntPtr self)\n"
                << "    {\n"
                << "        if (self == IntPtr.Zero)\n"
                << "            return null;\n"
                << "\n"
            ...
            ostr << "\n"
                ...
                << "    {\n"
                << "        if (self == IntPtr.Zero)\n"
                << "            throw new Exception(\"" << cls->name << ".Wrap cannot wrap null pointer\");\n"
                << "\n"
    ...
    if (!cls->baseClassName.empty())
    {
        ...
        // BaseWrap can return a null pointer without throwing an exception
        ostr << "\n"
            ...
            << "            return null;\n"
            << "\n"
            // Note that Wrap will do another dynamic_cast (unfortunately)
            << "        return Wrap(self);\n"
            << "    }\n";
    }
    ...
    if (service->nullable())
    {
        // the first two versions of VectorFromArray and VectorToArray handle the case that elements of the array cannot be null
    ...
    ostr << "\n"
        ...
        ostr << "\n"
            ...
            << "        for (int i = 0; i < size; ++i)\n"
            << "        {\n"
            // no need to test array[i] for null
        ...
        ostr << "\n"
            ...
            << "            throw spi.ErrorToException();\n"
            << "        }\n"
            // no need to test for null so we use .WrapNullable instead of .Wrap
            << "        array[i] = " << cls->name << ".WrapNullable(item);\n"
            << "    }\n"
            << "    return array;\n"
            << "}\n";
    }
    ...
    // FIXME: add Matrix functions for each class
}
    ...
    const spdoc::Enum* enumType) const
{
    ...
    ostr << "\n"
        ...
        << "    spi.PointerHandle h = new spi.PointerHandle(v, " << cname << "_Vector_delete);\n"
        << ""
        << "    if (!(array is null) && size > 0)\n" // preventing C# warnings
        << "    {\n"
        << "        int[] data = new int[size];\n"
        << "        for (int i = 0; i < size; ++i)\n"
        << "            data[i] = (int)array[i];\n"
        << "        if (spi_Enum_Vector_set_data(v, size, data) != 0)\n"
        << "        {\n"
        << "            throw spi.ErrorToException();\n"
        << "        }\n"
        << "    }\n"
        ...
        << "    {\n"
        << "        int[] data = new int[size];\n"
        << "        if (spi_Enum_Vector_get_data(v, size, data) != 0)\n"
        << "        {\n"
        << "            throw spi.ErrorToException();\n"
        << "        }\n"
        << "        for (int i = 0; i < size; ++i)"
        << "        {\n"
        << "            array[i] = (" << enumType->name << ")data[i];\n"
        << "        }\n"
        << "    }\n"
        << "    return array;\n"
        << "}\n";
    ...
    ostr << "\n"
        ...
        << "            }\n"
        << "        }\n"
        << "        if (spi_Enum_Matrix_set_data(m, nr, nc, data) != 0)\n"
        << "        {\n"
        << "            throw spi.ErrorToException();\n"
        << "        }\n"
        << "    }\n"
        ...
        << "    {\n"
        << "        int[,] data = new int[nr,nc];\n"
        << "        if (spi_Enum_Matrix_get_data(m, nr, nc, data) != 0)\n"
        << "        {\n"
        << "            throw spi.ErrorToException();\n"
        << "        }\n"
        << "        for (int i = 0; i < nr; ++i)"
        ...
        << "            for (int j = 0; j < nc; ++j)\n"
        << "            {\n"
        << "                array[i,j] = (" << enumType->name << ")data[i,j]; \n"
        << "            }\n"
        << "        }\n"
        << "    }\n"
        << "    return array;\n"
...
std::string CDataType::csType(int arrayDim, bool isOptional, bool inputArg) const
{
    std::string scalarType;
    std::ostringstream oss;

    const char* nullable = service->nullable() ? "?" : "";
    const char* optionalArray = (service->nullable() && inputArg) ? "?" : "";
    ...
    case 0:
        return scalarType;
    case 1:
        return scalarType + "[]" + optionalArray;
    case 2:
        return scalarType + "[,]" + optionalArray;
    default:
